...
public class SallePiste {
	...
	public synchronized void prendrepiste(Client c) {
		if(c.getGroupe().setTicket(ticket)){
			ticket++;
		}
		// tant qu'il n'y a pas de piste libre et que je n'ai pas de piste attribué --> je danse
		while ((c.getGroupe().getPisteReservee() == null && c.getGroupe().getTicket()!=tour) ||  (!pisteLibre() && c.getGroupe().getPisteReservee() == null && c.getGroupe().getTicket()==tour)) { 
			danser(c);
			
		}

		if (c.getGroupe().getPisteReservee() == null) { 	// si le groupe n'a pas de piste, un client du groupe prend une piste libre
			tour++;
			
			int pisteLibre = quellePisteLibre();
			System.out.println("Client : "+c.getNom() +" Groupe "+c.getGroupe().getNom()+" Action : Prendre Piste "+pisteLibre);
			System.out.println("C'est le tour du ticket "+tour);
			PisteBowling p = pistes.get(pisteLibre);
			p.entrerPiste(c);
			c.getGroupe().setNumPiste(p);
			
		} else {									// sinon le client rejoint son groupe sur la piste
			System.out.println("Client : "+c.getNom() +" Groupe "+c.getGroupe().getNom()+" Action : Rejoindre Piste "+c.getGroupe().getPisteReservee().getNumero());
